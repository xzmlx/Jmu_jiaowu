...
if(! isset($_SESSION['username']))
{
	...
	$conn = mysqli_connect('localhost','root','root','xk');//include("../conn/db_conn.php");
	include("../conn/db_func.php");
	$StuNo=$_SESSION['username'];
	$ShowCourse_sql="select * from course,stucou where course.Teacher=(select TeaName from Teacher where TeaNo='$TeaNo') and course.CouNo=stucou.CouNo ORDER BY StuNo";
...
<?php
if(db_num_rows($ShowCourseResult,$conn)>0){
	$number=db_num_rows($ShowCourseResult,$conn);
	if(empty($_GET['p']))
	$p=0;
	else {$p=$_GET['p'];}	
	$check=$p +10;
